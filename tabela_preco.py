...
if "carregou_uma_vez" not in st.session_state:
    ...
    st.cache_data.clear()
    st.session_state.ultima_atualizacao = (datetime.now() - timedelta(hours=3)).strftime("%d/%m/%Y %H:%M:%S")

# botão de atualizar (limpa cache e recarrega)
def atualizar_dados():
    st.cache_data.clear()
    hora_brasil = (datetime.now() - timedelta(hours=3)).strftime("%d/%m/%Y %H:%M:%S")
    st.rerun()
